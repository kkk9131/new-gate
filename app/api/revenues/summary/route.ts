 ...
 * - group_by: グループ化方法（'month' | 'project'）
 */
  ...
  try {
    ...
    if (groupBy === 'project') {
      const byProject = data.reduce((acc, item) => {
        ...
        return acc;
      }, {} as Record<string, { project_name: string; amount: number; tax_amount: number; count: number }>);
    ...
    return NextResponse.json(
      { error: 'group_byパラメータは "month" または "project" を指定してください' },
      { status: 400 }
    );
  } catch (error) {
