...
### コアフロー
1. プロジェクト一覧は **カード/リスト** をトグル可能（ユーザー設定に保存）。
2. フィルター: ステータス（active/completed/on_hold）、期間（start_date, end_date）、担当者/タグ（将来拡張）。
3. CRUD: 新規作成モーダル、カード内「…」メニューから編集/アーカイブ、削除はソフトデリート（`is_deleted`）。
4. 進捗表示: カードビューは円グラフ（進捗%ラベル付き）、リストビューは水平プログレスバー。
5. 添付/ドキュメント: 各プロジェクトにファイルサムネイルを最大5件表示。ドラッグ&ドロップ or 「+ ファイル」操作で追加。
6. 期日管理: `start_date` / `end_date` バッジ表示。期限切れはアクセントカラーで警告。
7. チャット依頼: 各プロジェクトから「AIに依頼」ボタンで Chat UI を呼び出し、自然言語コマンドを API キューに登録。

### UI構成
- ヘッダー: タイトル / ビュートグル（カード↔リスト） / 並び替えセレクタ / 「AIに依頼」 / 「新規プロジェクト」
- カードビュー: 2-3列グリッド、上部に名称＋ステータス、中央に円グラフ、下部に期間・添付チップ・アクション。
- リストビュー: テーブル状（名前/期間/担当/ステータス/進捗バー/添付/アクション）。
- 添付エリア: ファイルアイコン＋拡張子、ホバーで「プレビュー/ダウンロード」。
- Empty state: 「まだプロジェクトがありません。最初のプロジェクトを作成しましょう。」+ CTA。
- カラーパレット: Base Mist 背景 + Ink 文字。アクセントは進捗（Accent Calm）、期限超過（Accent Warm）に限定。
...
- Chat/AI: Phase7 で `chat_agent_tasks` API を実装し、プロジェクトID・操作種別・自然言語命令を渡す。
